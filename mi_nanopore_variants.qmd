---
title: "mi_nanopore_variants"
format: html
editor: visual
---

## **1. Dependencies and Argument Parsing**

-   Loads bioinformatics libraries for sequence handling:
...
-   Parses command line arguments: input/output folders, reference file, number of CPU cores, regions file, minimum coverage threshold.

-   The R code `args <- commandArgs()` means that we are calling the `commandArgs()` function to retrieve the **command-line arguments** that were provided when the R script was executed from a terminal or shell, and assigning them to a character vector variable named `args`
...
If more than one core is specified, it creates a fork cluster for potential parallel processing
...
## **3. Reference and Data Input**

-   Loads the reference genome.

-   Reads input files from the sequencing folder (`freqs.csv`, `codonFreqs.csv`).
...
## **4. Alignment and NGS FASTA Generation**

Uses `samtools tview` to display the alignment as text, reads this alignment, and writes it as FASTA sequences.

```{r}
#| label: FASTA generation
...
## **5. Gene Regions and Mapping**

-   Counting mapped reads in genomic regions from BAM files.

-   Generating a **mapping table** showing coverage per gene.
...
## **6. Coverage Calculation & Visualization**

**Visual output:** a wide genome-wide coverage plot where height indicates the number of reads at each position.
...
## **7. Consensus Sequence Construction**

-   Defines a function to map sets of nucleotides to IUPAC codes for ambiguous bases.

-   For each gene, and for various frequency thresholds (1%, 2%, 5%, 10%, etc.):

    -   Builds consensus DNA and amino acid sequence(s).

    -   Writes results to FASTA files.

-   Repeats the consensus construction for the entire genome, not just regions.
...
## **8. Amino Acid Frequency Matrices**

-   Transforms nucleotide codon frequencies to amino acid frequencies.

-   Writes out frequency tables for each region.

```{r}
#| label: AA frequency tables
...
After running the pipeline, you will find all the generated data in the **Nanopore output folder**. The files are organized by type, and you should see something like the following:

**Output File Name Patterns**

+---------------------------+----------------------------------------+
| **Type**                  | **Filename Example(s)**                |
+===========================+========================================+
| Consensus FASTA (region)  | `VP1_consensus_10pct.fasta`            |
|                           |                                        |
|                           | `VP1_consensus.fasta`                  |
+---------------------------+----------------------------------------+
| Consensus FASTA (genome)  | `Consensus_10pct.fasta`                |
|                           |                                        |
|                           | `Consensus.fasta`                      |
+---------------------------+----------------------------------------+
| NGS sequence              | `ngs.txt`                              |
|                           |                                        |
|                           | `ngs.fasta`                            |
+---------------------------+----------------------------------------+
| Coverage report           | `coverage.pdf`                         |
+---------------------------+----------------------------------------+
| Mapping summary           | `mapping-tab-date-lab.csv`             |
+---------------------------+----------------------------------------+
| Nuc freq (all/region)     | `calls.samplefreqs.csv`                |
|                           |                                        |
|                           | `VP1_calls.samplefreqs.csv`            |
+---------------------------+----------------------------------------+
| Codon freq (all/region)   | `calls.samplecodonFreqs.csv`           |
|                           |                                        |
|                           | `VP1_calls.samplecodonFreqs.csv`       |
+---------------------------+----------------------------------------+
| AA freq (region)          | `VP1_calls.sampleAAFreqs.csv` (varies) |
+---------------------------+----------------------------------------+
| Alignment (RDS)           | `pwalign.rds`                          |
+---------------------------+----------------------------------------+
| Nucleotide frequency XLSX | `nucleotide_frequency.xlsx`            |
+---------------------------+----------------------------------------+
| Zipped output             | `zipped/` (folder)                     |
+---------------------------+----------------------------------------+

**Note:**

-   `<region_name>` will be filled in with your actual gene/region name (e.g., VP1, Large_T, etc).

-   `calls.samplefreqs.csv` and similar are derived from input files and scripts may add region prefixes.

-   If you define different thresholds or have more/less regions, you’ll see corresponding output files generated.
